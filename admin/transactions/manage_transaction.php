...
if(isset($_GET['id'])){
    ...
    if($qry->num_rows > 0){
        ...
        echo '<script> alert("ID de transação desconhecido."); location.replace("./?page=transactions"); </script>';
    }
}
?>
    ...
    <div class="container-fluid">
        <div class="card card-outline card-outline rounded-0 shadow blur">
            <div class="card-header">
                <h5 class="card-title"><?= isset($id) ? "Atualizar ". $code . " Transação" : "Nova Transação" ?></h5>
            </div>
            <div class="card-body">
                <div class="container-fluid">
                    <form action="" id="transaction-form">
                        ...
                        <div class="row">
                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                <div class="form-group mb-3">
                                    <label for="client_name" class="control-label">Nome Completo do Cliente</label>
                        ...
                        <div class="row">
                            <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                                <div class="form-group mb-3">
                                    <label for="contact" class="control-label">Telefone / WhatsApp</label>
                                    ...
                                    <div class="row align-items-end">
                                        <div class="col-lg-10 col-md-10 col-sm-12 col-xs-12">
                                            <div class="form-group mb-0">
                                                <label for="service_sel" class="control-label">Selecione o Serviço</label>
                                        ...
                                        <div class="col-lg-2 col-md-2 col-sm-12 col-xs-12">
                                            <button class="btn btn-default bg-gradient-navy btn-sm rounded-0" type="button" id="add_service"><i class="fa fa-plus"></i> Adicionar</button>
                                        </div>
                                    </div>
                            ...
                            <div class="col-lg-7 col-md-7 col-sm-12 col-xs-12">
                                <fieldset>
                                    ...
                                    <div class="row align-items-end">
                                        <div class="col-lg-10 col-md-10 col-sm-12 col-xs-12">
                                            <div class="form-group mb-0">
                                                <label for="product_sel" class="control-label">Selecione o Produto</label>
                                        ...
                                        <div class="col-lg-2 col-md-2 col-sm-12 col-xs-12">
                                            <button class="btn btn-default bg-gradient-navy btn-sm rounded-0" type="button" id="add_product"><i class="fa fa-plus"></i> Adicionar</button>
                                        </div>
                                    </div>
                                    <div class="clear-fix mb-2"></div>
                        ...
                        <h2 class="text-navy text-right">Valor total a pagar: <b id="amount"><?= isset($amount) ? format_num($amount) : "0,00" ?></b></h2>
                        <hr>
                            ...
                            <div class="row">
                                <div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
                                    <select name="mechanic_id" id="mechanic_id" class="form-control form-control rounded-0">
                                        ...
                                        <option value="" <?= isset($mechanic_id) && in_array($mechanic_id,[null,""]) ? "selected" : "" ?>>Desconfigurado</option>
                                        <?php 
            ...
            <div class="card-footer py-2 text-right">
                <button class="btn btn-primary rounded-0" form="transaction-form">Salvar transação</button>
                <?php if(!isset($id)): ?>
                <a class="btn btn-default border rounded-0" href="./?page=transactions">Cancelar</a>
                <?php else: ?>
                <a class="btn btn-default border rounded-0" href="./?page=transactions/view_details&id=<?= $id ?>">Cancelar</a>
                <?php endif; ?> 
            </div>
        </div>
    </div>
...
<script>
    function calc_total_amount(){
        ...
        $('#amount').text(parseFloat(total).toLocaleString('pt-BR'))
    }
    function calc_service(){
        ...
        $('#service_total').text(parseFloat(total).toLocaleString('pt-BR'))
        calc_total_amount()
    }
    function calc_product(){
        ...
        $('#product_total').text(parseFloat(total).toLocaleString('pt-BR'))
        calc_total_amount()
    }
    $(function(){
        $('select#mechanic_id').select2({
            placeholder:"Selecione o Mecânico aqui",
            width:'100%',
            containerCssClass:'form-control form-control-sm rounded-0'
        })
        $('#service_sel').select2({
            placeholder:"Selecione o Serviço aqui",
            width:'100%',
            containerCssClass:'form-control form-control-sm rounded-0'
        })
        $('#product_sel').select2({
        ...
        $('#service-list tbody tr').find('.rem-service').click(function(){
            ...
            if(confirm("Tem certeza de que deseja remover "+(tr.find('.service_name').text())+" da lista de serviços?") === true){
                tr.remove()
                calc_service()
            }
        })
        $('#product-list tbody tr').find('.rem-product').click(function(){
            var tr = $(this).closest('tr')
            if(confirm("Tem certeza de que deseja remover "+(tr.find('.product_name').text())+" da lista de produtos?") === true){
                tr.remove()
                calc_product()
            }
        })
        ...
        $('#add_service').click(function(){
            ...
            if($('#service-list tbody tr input[name="service_id[]"][value="'+id+'"]').length > 0){
                alert("Serviço já está na lista.")
                return false;
            }
            ...
            tr.find('.rem-service').click(function(){
                if(confirm("Tem certeza de que deseja remover "+name+" da lista de serviços?") === true){
                    tr.remove()
                    calc_service()
                }
            })
        ...
        $('#add_product').click(function(){
            ...
            tr.find('.rem-product').click(function(){
                if(confirm("Tem certeza de que deseja remover "+name+" da lista de produtos?") === true){
                    tr.remove()
                    calc_product()
                }
            })
