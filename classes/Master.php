...
Class Master extends DBConnection {
	...
	function delete_service(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Serviço excluído com sucesso.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_mechanic(){
		...
		if($save){
			...
			$resp['status'] = 'success';
			if(empty($id))
				$resp['msg'] = "Novo mecânico salvo com sucesso.";
			else
				$resp['msg'] = " Mecânico atualizado com sucesso.";
	...
	function delete_mechanic(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Mecânico excluído com sucesso.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_product(){
		...
		if($save){
			...
			$resp['status'] = 'success';
			if(empty($id))
				$resp['msg'] = "Novo produto salvo com sucesso.";
			else
			...
			if(!empty($_FILES['img']['tmp_name'])){
				...
				if(!in_array($_FILES['img']['type'],$accept)){
					$resp['msg'] .= "O tipo de arquivo de imagem é inválido";
				}
	...
	function save_inventory(){
		...
		if($save){
			...
			$resp['status'] = 'success';
			if(empty($id))
				$resp['msg'] = "Novo estoque foi salvo com sucesso.";
			else
				$resp['msg'] = " O estoque foi atualizado com sucesso.";
	...
	function delete_inventory(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," O estoque foi excluído com sucesso.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function delete_product(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Produto excluído com sucesso.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_transaction(){
		...
		if($save){
			...
			$resp['status'] = 'success';
			if(empty($id))
				$resp['msg'] = "Nova transação salva com sucesso.";
			else
				$resp['msg'] = " Transação atualizada com sucesso.";
			if(isset($service_id)){
				$data = "";
				...
				if(!empty($data)){
					...
					if(!$save_services){
						...
						$resp['error'] = $this->conn->error;
						if(empty($id)){
							$resp['msg'] = "A transação falhou ao salvar.";
							$this->conn->query("DELETE FROM `transaction_services` where transaction_id = '{$tid}'");
						}else{
							$resp['msg'] = "A transação falhou na atualização.";
						}
						return json_encode($resp);
					}
				}
	...
	function delete_transaction(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Transação Deletada com Sucesso.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function update_status(){
		...
		if($update){
			...
			$resp['status'] = 'failed';
			$resp['msg'] = "O status da transação não foi atualizado.";
		}
		if($resp['status'] == 'success')
			$this->settings->set_flashdata('success', 'O status da transação foi atualizado com sucesso.');
		return json_encode($resp);
	}
}
